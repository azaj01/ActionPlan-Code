...
def _default_human_joints_pkl_path() -> str:
    """``<repo_root>/deps/human_joints_info.pkl`` (copied from ``zmq_post``)."""
    repo_root = Path(__file__).resolve().parents[3]
    return str(repo_root / "deps" / "human_joints_info.pkl")
...
def compute_human_joints(
    ...
    if human_joints_info is None:
        path = human_joints_info_path or _default_human_joints_pkl_path()
        if not os.path.isfile(path):
            raise FileNotFoundError(
                f"human_joints_info.pkl not found at {path}. "
                "Expected bundled copy under deps/ (see README)."
            )
        try:
            human_joints_info = torch.load(path, map_location="cpu", weights_only=False)
        except TypeError:
            human_joints_info = torch.load(path, map_location="cpu")
