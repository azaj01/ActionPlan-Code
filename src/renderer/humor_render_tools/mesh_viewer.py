# Adapted from STMC: src/renderer/humor_render_tools/mesh_viewer.py
# Upstream repo: https://github.com/nv-tlabs/stmc
# Source file: https://github.com/nv-tlabs/stmc/blob/main/src/renderer/humor_render_tools/mesh_viewer.py
...
class MeshViewer(object):
    def __init__(
        ...
        self.text_overlay_seq = None
        self.text_color = (0, 0, 0)
        self.text_font_scale = 1.0
        self.text_thickness = 2
        self.text_margin = 20

        # legend overlay (list of {"color": [r,g,b], "label": str})
        self.legend_entries = None
        self.legend_margin = 20
        self.legend_swatch_size = 18
        self.legend_row_spacing = 8
        self.legend_font_scale = 0.6
    ...
    def set_text_overlay(self, text, color=(0, 0, 0), font_scale=1.0, thickness=2, margin=20):
        self.text_overlay = str(text)
        self.text_color = tuple(color)

    def set_text_overlay_seq(self, text_seq):
        """Per-frame text overlay. text_seq: list/array of str, one per frame."""
        self.text_overlay_seq = list(text_seq) if text_seq is not None else None

    def set_legend(self, legend_entries, margin=20, swatch_size=18, row_spacing=8, font_scale=0.6):
        ...
        try:
            ...
            for e in legend_entries:
                ...
                c = np.asarray(color).astype(float)
                if c.max() > 1.0:
                    c = c / 255.0
                c = np.clip(c, 0.0, 1.0)
                entries.append({"color": c, "label": label})
            if len(entries) == 0:
                self.legend_entries = None
            else:
                self.legend_entries = entries
                self.legend_margin = int(margin)
                self.legend_swatch_size = int(swatch_size)
                self.legend_row_spacing = int(row_spacing)
                self.legend_font_scale = float(font_scale)
        except Exception:
            self.legend_entries = None

    def _wrap_text_lines(self, text, font, font_scale, thickness, max_width, cv2):
        words = text.split()
        lines = []
        cur = ""
        for w in words:
            candidate = w if cur == "" else cur + " " + w
            (w_size, _) = cv2.getTextSize(candidate, font, font_scale, thickness)
            if w_size[0] <= max_width:
                cur = candidate
            else:
                if cur == "":
    ...
    def render(self):
        ...
        # Draw text overlay with OpenCV if requested
        txt = None
        if self.text_overlay_seq is not None and 0 <= self.animation_frame_idx < len(self.text_overlay_seq):
            txt = self.text_overlay_seq[self.animation_frame_idx]
        elif self.text_overlay is not None:
            txt = self.text_overlay
        if txt is not None:
            try:
                ...
                # draw with wrapping to stay within margins
                font = cv2.FONT_HERSHEY_SIMPLEX
                margin = self.text_margin
                max_w = max(10, bgr.shape[1] - 2 * margin)
                max_h = max(10, bgr.shape[0] - 2 * margin)
                font_scale = self.text_font_scale

                # adapt font scale if needed to fit height
                for _ in range(5):  # few iterations to adjust
                    lines = self._wrap_text_lines(txt, font, font_scale, self.text_thickness, max_w, cv2)
                    # estimate total height
                    line_height = cv2.getTextSize("Ag", font, font_scale, self.text_thickness)[0][1]
                    total_h = len(lines) * (line_height + 6)  # 6px spacing
                    if total_h <= max_h:
                        break
                    font_scale *= 0.9
                ...
                bg_top = margin - 8
                bg_bottom = margin + total_h + 8
                bg_left = margin - 8
                bg_right = margin + max([cv2.getTextSize(l, font, font_scale, self.text_thickness)[0][0] for l in lines] + [0]) + 8
                bg_top = max(0, bg_top)
                bg_left = max(0, bg_left)
                bg_bottom = min(bgr.shape[0]-1, bg_bottom)
                bg_right = min(bgr.shape[1]-1, bg_right)
                overlay = bgr.copy()
                cv2.rectangle(overlay, (bg_left, bg_top), (bg_right, bg_bottom), (255, 255, 255), thickness=-1)
                alpha = 0.35
                ...
                if output_img.shape[-1] == 4:
                    output_img = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGBA)
                else:
                    output_img = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
            except Exception:
                pass
        ...
        if self.legend_entries is not None:
            try:
                ...
                sw = self.legend_swatch_size
                row_space = self.legend_row_spacing
                font_scale = self.legend_font_scale
                thickness = 1

                # Compute max label width to optionally draw bg
                text_widths = []
                for e in self.legend_entries:
                    (tw, th) = cv2.getTextSize(e["label"], font, font_scale, thickness)[0]
                    text_widths.append(tw)
                max_tw = max(text_widths + [0])
                total_h = len(self.legend_entries) * (max(12, int(font_scale * 16)) + row_space) + margin
                total_w = sw + 8 + max_tw + 2 * margin

                # Position at top-right
                x0 = max(0, bgr.shape[1] - total_w)
                y0 = margin

                # Background box for readability
                overlay = bgr.copy()
                cv2.rectangle(overlay, (x0, y0), (bgr.shape[1] - 1, y0 + total_h), (255, 255, 255), thickness=-1)
                alpha = 0.35
                bgr = cv2.addWeighted(overlay, alpha, bgr, 1 - alpha, 0)

                y = y0 + margin
                for e in self.legend_entries:
                    c = (e["color"] * 255.0).astype(np.uint8).tolist()
                    # swatch
                    cv2.rectangle(bgr, (x0 + margin, y), (x0 + margin + sw, y + sw), (int(c[2]), int(c[1]), int(c[0])), thickness=-1)
                    # label
                    cv2.putText(
                        bgr,
                        e["label"],
                        (x0 + margin + sw + 8, y + int(sw * 0.8)),
                        font,
                        font_scale,
